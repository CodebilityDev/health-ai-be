...
import { SYS } from "~modules/chatai/openai/healthbot/utils";
...
export const WebhookRoutines = {
  ...
  onNewMessage: async (args: {
    ...
    if (groupData?.enable_profileBuilder) {
      (async () => {
        ...
        let customContextList = (groupData?.user_contextFields ||
          []) as string[];
        ...
        await getGHLContactUpdate({
          ...
          contactId: args.contactID,
          groupID: args.groupID,
          updateData: updatedUserInfoPayload,
        });
      })();
    }
